...
static std::tuple<float, float, float> computeBarycentric2D(float x, float y, const Vector3f* v,float alpha,float beta,float gamma)
{
...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    for(int i=left;i<=right;i++){
        for(int j=bottom;j<=top;j++){ 
            int num=0;
            float zvalue=0xfffff;
            for(int k=0;k<4;k++){
                if(insideTriangle(i+pos[k][0],j+pos[k][1],t.v)){
                    computeBarycentric2D(i,j,t.v,alpha,beta,gamma);
                    float w=1.0/(alpha/v[0].w()+beta/v[1].w()+gamma/v[2].w());
                    float z=alpha *v[0].z()/v[0].w() + beta * v[1].z()/v[1].w() + gamma*v[2].z()/v[2].w();
                    z*=w;
                    zvalue=std::min(zvalue,z);
                    num++;
                
                }
            }
            if(num){
                    if(zvalue<depth_buf[get_index(i,j)]){
                        depth_buf[get_index(i,j)]=zvalue;
                        set_pixel(Eigen::Vector3f(i,j,1),t.getColor()*num/4);
                    }
            }
        }
    }
